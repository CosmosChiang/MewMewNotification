...
| 2 | 桌面通知直接開啟 issue、標記已讀（已完成；待各平台實機驗收） | Profile-scoped notification mapping（已完成） |
...
桌面通知直接開啟 issue 與明確標記已讀已完成。現行實作包含 Profile-scoped opaque mapping、點擊時 URL/record/Profile 驗證、批次通知 Popup fallback、兩個平台按鈕、冪等本機已讀更新、mapping 期限與上限，以及 Chrome 拒絕建立通知時的 mapping rollback。OS 通知按鈕的實際呈現仍依平台而異，發布前須依 `docs/DESKTOP_NOTIFICATION_ACTIONS.md` 完成 Windows 與目標平台實機清單。
