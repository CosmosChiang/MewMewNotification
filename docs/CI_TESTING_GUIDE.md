...
- 執行 `npm ci`
- 僅在 Node.js `20.x` 執行 `npm run audit:high`
- 執行 `npm run test:ci`
...
- CI 目前以 `npm audit --audit-level=high` 作為阻擋門檻
- 本次安全收斂已清除 direct 與 high-risk 問題
- 剩餘 audit 項目為低風險的 Jest / jsdom 工具鏈漏洞
- 這些低風險項目需要跨 major 升級 Jest / jsdom / `http-proxy-agent`
  才能完全消除，因此目前先保留並持續追蹤
- 若未來要完全清空 audit，應優先規劃 Jest 27 -> 30 與
  jsdom 16 -> 29 的相容性升級
