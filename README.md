...
使用前請閱讀 [MewMewNotification 隱私權政策](https://github.com/CosmosChiang/MewMewNotification/blob/main/PRIVACY.md)，其中說明 API 金鑰、Redmine 議題資料、本機儲存、資料保留與刪除方式。
...
- 通知聚焦控制：支援專案納入/排除規則、變更類型篩選、安靜時段與短時間通知合併
- 自動偵測議題更新，智慧通知管理
- 徽章計數器顯示未讀通知數量
...
- 每張議題卡片都可展開「更多操作」：
  ...
  - 操作可用性會跟隨 Redmine 權限；若角色沒有權限，介面會明確顯示提示，而不
    是靜默失敗。
- 支援通知聚焦控制：
  - **專案規則**：可選擇所有專案皆通知，或只通知納入/排除後的專案。
  - **變更類型篩選**：可分別控制狀態、指派人員、優先權、留言/日誌活動與一般更新是否要出現。
  - **安靜時段**：在指定時段抑制桌面通知，但保留未讀與通知歷史，稍後可回來集中處理。
  - **短時間通知合併**：同一議題在短時間內連續更新時，會合併成單一保留通知並累積摘要。
- 可手動重新整理通知
- 支援通知失敗重試與錯誤提示
...
4. 修改程式後可直接重新整理擴充功能。
5. 打包發佈：
    - 在 PowerShell 執行 `.\tools\package-extension.ps1`
    - 會產生 `dist\mewmew-notification-extension.zip`，內容僅包含 Chrome 擴充功能執行所需檔案
...
- `npm run audit:moderate`：檢查 moderate / high / critical 依賴漏洞
- GitHub Actions 會在 `main` 的 push 與對 `main` 的 PR 上執行多 Node 版本測試、驗證與擴充功能打包
- GitHub Actions 會在 Node.js `22.x` 與 `24.x` job 執行 `npm run audit:moderate`，阻擋中度以上依賴漏洞
...
- `alarms`：依設定的間隔排程通知同步
- `background`：執行 Manifest V3 service worker
...
- 該議題所屬專案沒有被排除，或已被納入允許清單
- 對應的變更類型沒有被停用
- 目前時間不在安靜時段內（安靜時段仍會保留通知歷史）
