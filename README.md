...
3. 在設定頁面輸入你的 Redmine 伺服器網址（Redmine URL）及 API 金鑰（API Key）。
   ...
   - 遠端 Redmine 伺服器必須使用 HTTPS。
   - `http://localhost` 與 `http://127.0.0.1` 僅允許用於本機開發測試，且會顯示不安全連線警告。
...
**⚠️ 安全提醒**: 遠端連線一律要求 HTTPS；僅 `localhost` 與 `127.0.0.1` 可在本機開發時使用 HTTP，且會顯示警告
...
- **傳輸安全**：遠端伺服器強制使用 HTTPS，僅本機開發例外允許 HTTP
